...
@section('content')
    ...
    @if ((isset($_GET["usuario_eliminado"]) && $_GET["usuario_eliminado"]) || (session()->has('usuario_eliminado')))
        <script src="//cdn.jsdelivr.net/npm/sweetalert2@11"></script>
        <script type="text/javascript">
            const Toast2 = Swal.mixin({
                toast: true,
    ...
    <div class="container px-4">
        ...
        <table class="border table table-hover rounded empleados">
                        ...
                        <td>
                            <div class="row">
                                @if ($usuario->rol == "tecnico")
                                    <a href="{{ route('tecnico.tareas', ['tecnico_codigo' => $usuario->puesto->codigo]) }}" class="col-3 empleados" data-bs-toggle="tooltip" data-bs-placement="top" title="Historial"> <!-- Ver historial tareas -->
                                ...
                                <a href="#eliminar-{{ $usuario->id }}" onclick="enviarFormDelete({{ $usuario->id }})" class="col-3 empleados" data-bs-toggle="tooltip" data-bs-placement="top" title="Eliminar"> <!-- Eliminar empleado -->
            ...
            <form class="d-none" action="{{ route('empleados.delete') }}" method="post">
                @csrf
                {{ method_field('DELETE') }}
                <input id="empleado-delete" name="id" type="submit" value="id">
            </form>
            ...
            <script type="text/javascript">
                function enviarFormDelete(id) {
                    // console.log(id);
                    let boton = document.getElementById('empleado-delete');
                    boton.value = id;
                    boton.click();
                }
            </script>
